import "dotenv/config"
import { drizzle } from "drizzle-orm/neon-http"
import { neon } from "@neondatabase/serverless"

import * as schema from "../db/schema"

const sql = neon(process.env.DB_URL!);

const db = drizzle(sql, {schema});

const main = async () => {
  try {
    console.log("Seeding database");

    await db.delete(schema.courses);
    await db.delete(schema.userProgress);
    await db.delete(schema.units);
    await db.delete(schema.lessons);
    await db.delete(schema.challenges);
    await db.delete(schema.challengeProgress);
    await db.delete(schema.challengeOptions);
    await db.delete(schema.userSubscription);

    await db.insert(schema.courses).values([
      {
        id: 1,
        title: "Japanese",
        src: "/assets/jp.svg"
      },
      {
        id: 2,
        title: "Korea",
        src: "/assets/kr.svg"
      },
      {
        id: 3,
        title: "Malaysia",
        src: "/assets/my.svg"
      },
      {
        id: 4,
        title: "China",
        src: "/assets/cn.svg"
      },
      {
        id: 5,
        title: "United Kingdom",
        src: "/assets/uk.svg"
      }
    ])

    await db.insert(schema.units).values([
      {
        id: 1,
        courseId: 1,
        title: "Unit 1",
        description: "Introduction to Japanese",
        order: 1
      }
    ])

    await db.insert(schema.lessons).values([
      {
        id: 1,
        unitId: 1, // Unit 1
        order: 1,
        title: "Hiragana",
      },
      {
        id: 2,
        unitId: 1, // Unit 1
        order: 2,
        title: "Katakana",
      },
      {
        id: 3,
        unitId: 1, // Unit 1
        order: 3,
        title: "Kanji",
      },
      {
        id: 4,
        unitId: 1, // Unit 1
        order: 4,
        title: "Vocabulary",
      },
      {
        id: 5,
        unitId: 1, // Unit 1
        order: 5,
        title: "Grammar",
      }
    ])

    await db.insert(schema.challenges).values([
      {
        id: 1,
        lessonId: 1,
        type: "SELECT",
        question: "What is the word for 'car'?",
        order: 1
      },
      {
        id: 2,
        lessonId: 1,
        type: "ASSIST",
        question: "'car'",
        order: 3
      },
      {
        id: 3,
        lessonId: 1,
        type: "SELECT",
        question: "What is the word for 'cat'?",
        order: 2
      }
    ])

    await db.insert(schema.challengeOptions).values([
      {
        id: 1,
        challengeId: 1,
        imageSrc: '/assets/train.svg',
        correct: false,
        text: 'でんしゃ',
        audioSrc: '/assets/train.mp3'
      },
      {
        id: 2,
        challengeId: 1,
        imageSrc: '/assets/bicycle.svg',
        correct: false,
        text: 'じでんしゃ',
        audioSrc: '/assets/bicycle.mp3'
      },
      {
        id: 3,
        challengeId: 1,
        imageSrc: '/assets/car.svg',
        correct: true,
        text: 'くるま',
        audioSrc: '/assets/car.mp3'
      }
    ])

    await db.insert(schema.challengeOptions).values([
      {
        id: 4,
        challengeId: 2,
        correct: false,
        text: 'でんしゃ',
        audioSrc: '/assets/train.mp3'
      },
      {
        id: 5,
        challengeId: 2,
        correct: false,
        text: 'じでんしゃ',
        audioSrc: '/assets/bicycle.mp3'
      },
      {
        id: 6,
        challengeId: 2,
        correct: true,
        text: 'くるま',
        audioSrc: '/assets/car.mp3'
      }
    ])

    await db.insert(schema.challengeOptions).values([
      {
        id: 7,
        challengeId: 3,
        imageSrc: '/assets/train.svg',
        correct: false,
        text: 'いぬ',
        audioSrc: '/assets/train.mp3'
      },
      {
        id: 8,
        challengeId: 3,
        imageSrc: '/assets/bicycle.svg',
        correct: false,
        text: 'ねこ',
        audioSrc: '/assets/bicycle.mp3'
      },
      {
        id: 9,
        challengeId: 3,
        imageSrc: '/assets/car.svg',
        correct: true,
        text: 'うし',
        audioSrc: '/assets/car.mp3'
      }
    ])

    await db.insert(schema.challenges).values([
      {
        id: 4,
        lessonId: 2,
        type: "SELECT",
        question: "What is the word for 'car'?",
        order: 1
      },
      {
        id: 5,
        lessonId: 2,
        type: "ASSIST",
        question: "'car'",
        order: 3
      },
      {
        id: 6,
        lessonId: 2,
        type: "SELECT",
        question: "What is the word for 'cat'?",
        order: 2
      }
    ])

    await db.insert(schema.quests).values([
      {
        title: "Earn 100 points",
        points: 100,
        expiredTime: new Date("2024-08-10"),
      },
      {
        title: "Earn 200 points",
        points: 200,
        expiredTime: new Date("2024-08-12"),
      },
      {
        title: "Earn 500 points",
        points: 500,
        expiredTime: new Date("2024-08-14"),
      }
    ])

    console.log("Seeding finished")
  } catch (error) {
    console.error(error)
    throw new Error("Failed to seed the database");
  }
}

main()